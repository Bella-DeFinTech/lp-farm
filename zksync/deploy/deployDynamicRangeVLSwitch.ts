...
export default async function (hre: HardhatRuntimeEnvironment) {
  ...
  const useOriginLiquidityStr = farmConfig['useOriginLiquidity'] ?? "false"
  const useOriginLiquidity = useOriginLiquidityStr === 'true' ? true : false
  ...
  const contractFactory = await deployer.loadArtifact("DynamicRangeVLSwitchTimestamp");
